...
| 구분 | 담당 |
|------|------|
| 통신모듈 | RS-485 폴링 → **고정 바이너리** uplink, cmd downlink 수신·적용 |
| 수집서버 (RSD) | topic 농장 식별, CRC·decode, chunk 병합, 명령 MQTT publish |
| 대시보드 | LIVE·명령 UI, `ctrl_thermo_command` 저장 |
| 연계서버 | DB 이후 축평원 v4.0 필드 변환 (본 transport 범위 외) |
...
| P7 | **multi-chunk burst 키 = `session_id`** — 2 KB MQTT 버퍼 한계 대응 |
...
flowchart LR
  ...
  subgraph cloud [클라우드]
    R[R.py] --> S[S.py] --> RAWDB[(raw)]
    RAWDB --> D[D.py] --> DEC[(decoded)]
    C[C.py] --> CMD
...
| 요소 | 설명 |
|------|------|
| **통신모듈** | 농장 1모듈. 최대 **48 컨트롤러** 스냅샷 (LIVE multi-chunk) |
| **컨트롤러** | 축사 내 장비 1~10 (`eqpmn_no`). 업무 키 **`controllerKey`** |
| **채널** | 컨트롤러 내 슬롯 **A / B / C** — 각각 독립 온·습·장비·측정·thermo |
| **농장** | `lsindRegistNo` + `itemCode` (topic) |
| **모듈** | 농장당 1대, DB **`module_uid=1`** (implicit) |
...
|------|-----|
| 주기 | **5분** 1회 (운영 기본) |
| 배치 | **모듈 1대 = burst 1회** (multi-chunk 가능) |
| 행 수 | 기본 **48** ctrl / burst |
| QoS | **1** (raw·cmd) |
...
| C.py publish | `wire_command.py` → MQTT, `payload_json` 저장 | `sent` |
| uplink 일치 | `channels[].thermo` vs sent 4값 | `applied` |
| TTL 만료 / dedupe | C.py | `cancelled` |
| encode/MQTT 오류 | C.py | `failed` |

**C.py dedupe 키:** `(lsind, item, module_uid, stall_ty, stall_no, eqpmn_no, channel, eqpmn_code)` — 동일 타깃 pending은 **최신 1건만** 전송.

**ACK 매칭 키:** `controllerKey|channel|eqpmnCode` (`command_ack.py`).
...
2. CRC 검증
3. Header 12B 파싱 → `session_id`, flags, `chunk_seq`
4. 각 row 77B → `controllerKey` + `channels[]`
5. `row_t` → `mesureDt`
6. LIVE: `last_chunk`까지 chunk 병합 (`session_id`)
7. controller dedupe: `controllerKey|mesureDt`
8. (선택) `APPLY_STALL_LUT` — SP 슬롯 → 축종 코드 remap

---

## 8. DB

### 8.1 `iot_room_state_raw` / `iot_room_state_decoded`
...
| `session_id` | burst ID |
| `lut_ver` | **null** |
| `mesure_dt` | optional (merged 대표 시각) |
| `chunk_seq` | 0…N |
| `mode` | `live` / `replay` |
| `decoded_json` | schema **2.0** |

### 8.2 `ctrl_thermo_command`
...
- LIVE: `decoded_json.controllers[].channels[]`
...
```
MQTT raw  → R.py  → Redis → S.py → iot_room_state_raw
raw       → D.py  → decode + live_chunk_merge → iot_room_state_decoded
            command_ack (sent → applied)
pending   → C.py  → MQTT cmd
```

| 프로세스 | 역할 |
|----------|------|
| `R.py` | MQTT `…/raw` subscribe → Redis |
| `S.py` | raw 테이블 insert |
| `D.py` | v0x0B decode, chunk merge, ACK hook |
| `C.py` | `ctrl_thermo_command` poll → `…/cmd` publish |

구현 참조: `RSD/wire_decode.py`, `live_chunk_merge.py`, `wire_command.py`, `C.py`, `command_ack.py`
...
| 항목 | 값 |
|------|-----|
| W5500 TX 버퍼 | ~2 KB → 패킷 **26 row** 상한 |
| 모듈당 ctrl | **48** (multi-chunk 필수) |
...
| 영역 | 경로 |
|------|------|
| Wire decode·encode | `SI1/RSD/wire_decode.py` |
| Chunk merge | `SI1/RSD/live_chunk_merge.py` |
| Downlink | `SI1/RSD/wire_command.py`, `C.py` |
| ACK | `SI1/RSD/command_ack.py` |
| DB migration | `SI1/dashboard/web/supabase/migrations/20260613000000_wire_v00b.sql` |
