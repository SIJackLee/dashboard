...
**계층**: `farm_uid` → `module_uid` → `controllers[idx]`
...
| 컨트롤러 메타데이터 / 축사 지정 저장 | 미구현 (저장 방식 미정) |

## 9. 주요 의사결정

- 신규 테이블 개설하지 않음. "축사 지정"은 추후 사용자가 설정탭에서 컨트롤러에 메타데이터를 부여하는 방식(저장 위치 미정).
- 현재 "축사" UI는 메타데이터 도입 전까지 **컨트롤러(idx) 단위**로 표시.
- 제어 명령 의도는 4종: **최저환기 / 최고환기 / 설정온도 / 온도편차** (`ctrl_thermo_command`).
- 인증은 OAuth가 아니라 이메일/비밀번호 (계정이 이미 `auth.users`에 존재).

## 10. 진행 중 / 대기 작업: 컨트롤러 제품 UI
...
web/src/
  ...
  lib/
    data/iot.ts            # decoded 파싱·집계 (핵심 데이터 레이어)
