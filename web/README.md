...
- **출고 스모크:** [`docs/SHIP_CHECKLIST.md`](docs/SHIP_CHECKLIST.md) — admin / operator / viewer 핵심 경로
...
- 데이터 접근 권한이 없으면 `/pending`, 관리자만 `/admin/ops`(스캔·디렉터리·명령) 접근.
- 로컬 테스트 계정·출고 체크 항목은 [`docs/SHIP_CHECKLIST.md`](docs/SHIP_CHECKLIST.md) 참고.
...
관리자 전용. 모바일에서는 **사용자 → 명령 → 스캔** 순, PC에서는 스캔이 위.
