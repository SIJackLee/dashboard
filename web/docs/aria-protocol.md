...
- **허브 뱃지(단독 인터페이스):** 필드·차트·모델 **콘텐츠 영역** 우측 하단(하단 탭 독 위). 차트에서는 브러시 위로 띄움. 이름 DELIN, 제목 + 축사유형별 짧은 목록(최대 3줄, 나머지는 외 N건). 전용 탭 없음. `view=aria`는 필드. 목록 탭에는 없음. 음성·적용 없음. 차트·필드·모델은 **축사유형** 단위(모델은 고른 동).
