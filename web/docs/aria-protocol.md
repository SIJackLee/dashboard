...
- **허브 뱃지(단독 인터페이스):** 필드·차트·모델 **콘텐츠 영역** 우측 하단(하단 탭 독 위). 차트에서는 브러시 위로 띄움. 기본은 아이콘만, 안내가 있으면 숫자, 누르면 말풍선(이름 DELIN, 제목 + 축사유형별 짧은 목록 최대 3줄, 나머지는 외 N건). 전용 탭 없음. `view=aria`는 필드. 목록 탭에는 없음. 음성·적용 없음. 차트·필드·모델은 **축사유형** 단위(모델은 고른 구역).
- **오브·TTS·말하기 도크는 은퇴.** DELIN은 사용자가 보는 화면에서 조언하는 **뱃지 BOT**만 쓴다. 음성 경로(`view=aria` 오브, 말하기 FAB, `/api/voice-report/*`)는 `VOICE_REPORT_ENABLED=false`로 끈다(코드는 보존).
- **뱃지 조언 우선순위:** 통신두절 → 장비 경보(안전망 임계) → 권장표 이탈 → 적정. 통신두절·장비경보는 danger 톤, 권장표 이탈은 warn 톤.

코드: `src/lib/farm/pig-env-recommend.ts` (`pigEnvBadgeAdvice` — 통신두절·안전망 임계 포함), `src/components/farm/delin-env-badge.tsx`.
