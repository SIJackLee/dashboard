...
채널 hue는 status와 겹치지 않게 둔다: temp ≈ 15(와인) · motor/fan-intake ≈ 260(건메탈) · hum/info 230 · **command 210(청록)** · exhaust 295 · supply 165. 모터는 정상/주의/위험(녹·황·적)을 쓰지 않는다. 채도는 채널 중(모터는 스틸감으로 약간 낮음) · 주의/위험 고. 차트는 `var(--channel-*)`만. 온도 설정±편차 밴드는 primary가 아니라 `color-mix(…, var(--channel-temp), var(--mix-lift))`로 **같은 색상각·명도만** 올린다. 명령 적중 점·적용 큐 진행은 `--channel-command` 농도(+ 확인 시 `--status-ok`).
