...
| 구분 | 담당 |
|------|------|
| 펌웨어 | 컨트롤러 수집 즉시 **고정 바이너리** uplink (`…/raw`) |
| **RS** | uplink **passthrough** → `iot_room_state_raw` (wire 미파싱) |
| **DB View** | raw `payload_bytea` → controller·채널·LIVE 최신 등 **조회용 정제** |
| **C** | `ctrl_thermo_command` → `…/cmd` downlink · uplink thermo로 **applied** 판정 |
| 연계서버 | View/조회 결과 → 축평원 v4.0 (DB 이후) |
...
| 요소 | 설명 |
|------|------|
| **통신모듈** | 마스터. RS-485로 컨트롤러 **순차 수집**, **1대 읽을 때마다** `…/raw` uplink 1건 (`n=1` row). 모듈당 최대 **48** ctrl |
| **RS** | MQTT `…/raw` subscribe → **`iot_room_state_raw` passthrough INSERT**. 실시간 디코더(**D**) **미사용** |
| **C** | `ctrl_thermo_command` pending → MQTT `…/cmd` JSON → `sent`. uplink thermo 일치 시 **`applied`** |
...
| 항목 | 값 |
|------|-----|
| 주기 | **5분** 1회 (운영 기본) — 1주기 = 모듈 전체 ctrl **순차 폴링** |
| uplink 단위 | **컨트롤러 1대 수집 완료 → MQTT 1건** (`header.n=1`, row 77 B 1개) |
| 1주기 row 수 | 최대 **48** raw row / 모듈 (ctrl 수와 동일) |
| QoS | **1** (`ver=0x05` 이상). raw·cmd 공통 |
| 레거시 burst | `n>1`·multi-chunk·`session_id` 묶음 — **호환 디코드만** (View/SQL). 신규 펌웨어는 row 스트림 우선 |
| Phase 3 (레거시) | topic `/KEY12/`, JSON — **단계적 폐기** |
...
- RSD subscribe: `sungil/+/+/raw` (**QoS 1**) — **RS** (목표: `RS.py`; 과도기 `R.py`+`S.py` passthrough 동등)
...
### 4.3 클라우드 파이프라인 (RS · DB · C)

#### 4.3.1 Uplink — RS → DB

| 단계 | 처리 | DB |
|------|------|-----|
| 1 | 통신모듈: ctrl RS-485 read → **즉시** `…/raw` publish | — |
| 2 | **RS:** MQTT body **그대로** INSERT | `iot_room_state_raw` |
| 3 | topic에서 `lsind_regist_no`, `item_code`, `module_uid=1`만 추출 | 동일 row |
| 4 | wire header·row **파싱하지 않음** | `wire_ver` 등 **null** |

**1주기(48 ctrl) 적재:** raw **48 row** (MQTT 48건 = INSERT 48건).

#### 4.3.2 조회 — Edge decode + View

| 객체 | 역할 |
|------|------|
| `iot_room_state_raw` | RS passthrough **정본** (append-only) |
| `iot_decode_cursor` | decode **증분 cursor** (`last_raw_id`) |
| `iot_room_state_decoded` | Edge **UPSERT** — v0x0C controller 1행 + `decoded_json` |
| `v_iot_decoded_latest` | `controller_key`별 **최신 1건** (LIVE) — **Dashboard 주 조회** |
| ~~`v_iot_raw_live`~~ | *(제거)* | Health insert rate는 **`iot_room_state_raw`** 직접 count |

**decode 실행:** Edge Function `decode-batch` — pg_cron **10초**, cursor batch (최종 프로토콜 v0x0C only).

대시보드 LIVE·알람은 **`v_iot_decoded_latest` SELECT** — **TS decode 없음**.

#### 4.3.3 Downlink — C → 통신모듈 → applied
...
sequenceDiagram
  ...
  UI->>DB: INSERT pending
  C->>DB: poll pending
  C->>CM: MQTT …/cmd JSON
  C->>DB: status=sent
  CM->>CM: 컨트롤러 thermo 적용
  CM->>RS: …/raw (n=1, thermo 반영)
  RS->>DB: raw INSERT
  ACK->>DB: thermo vs sent → applied
```

| status | 담당 | 조건 |
|--------|------|------|
| `pending` | 대시보드 | UI 저장 |
| `sent` | **C.py** | MQTT publish 성공 |
| `applied` | **command_ack** | uplink `channels[].thermo` 4값 ≡ sent 명령 |
| `failed` / `cancelled` | C.py | publish 오류 / TTL / dedupe |

**적용완료 사인:** 별도 ACK topic 없음. cmd 적용 후 **`/raw` uplink 1건**에 갱신된 thermo가 실리면 완료.

**ACK 매칭 키:** `controllerKey|channel|eqpmnCode` (예: `SP01:01:03|B|EC02`). 같은 장비코드를 여러 채널이 써도 `|A|EC02`와 `|B|EC02`로 구분한다. 장비코드만으로 ACK를 맞추면 안 된다.

#### 4.3.4 예시 데이터 (FARM01 · ctrl #3 · 채널 B 설정)

**① 통신모듈 → MQTT** (`sungil/FARM01/P00/raw`, `n=1`, ≈91 B)

```text
ver=0x0B  flags=0x00  session_id=*(임의)*  n=1  chunk_seq=0
row: eqpmn_no=03  stall SP01:01  row_t=→ 20260608140503 KST
     channels A/B/C — B 슬롯 thermo: 24.0 / 1.5 / 15% / 90%
CRC OK
```

**② RS → `iot_room_state_raw` (passthrough)**
...
```json
{
  ...
  "saved_at": "2026-06-08T05:05:03+00:00"
}
```

`wire_ver`, `session_id`, `chunk_seq`, `mode`, `crc_ok` — **컬럼 null** (RS 미기록).

**③ View `v_iot_controller_row` (정제 결과 예)**
...
```json
{
  ...
  "channels": [
    ...
    },
    {
      ...
      "humidityPct": "58.0",
      "outputs": { "2": "45" },
      "thermo": { "setpointTemp": "24.0", "tempDeviation": "1.5", "minVentPct": 15, "maxVentPct": 90 }
    }
  ]
}
```

**④ C.py → `ctrl_thermo_command` (downlink 직전·직후)**
...
C.py publish → MQTT `sungil/FARM01/P00/cmd`:
...
{
  "status": "sent",
  "sent_at": "2026-06-08T05:04:50+00:00",
  "payload_json": { "...": "위 MQTT body" }
}
```

**⑤ command_ack → `applied`**

uplink ②·View ③의 **B채널 thermo**가 sent 4값과 일치하면:

```json
{
  "status": "applied",
  "applied_at": "2026-06-08T05:05:04+00:00"
}
```

**⑥ LIVE 화면 (View `v_iot_decoded_latest`)**

모듈 FARM01/P00 기준 `controllerKey`별 최신 1건 — Edge decode 적재본 SELECT (TS decode 없음).
...
**유효 범위** (downlink `ctrl_thermo_command` CHECK · 대시보드와 동일):

- setpoint: 0~300 (0.0~30.0℃), UI 스텝 **0.1℃** (와이어 ×10). B·C 채널 값은 A 설정온도에 더해져 동작하므로 UI·명령 상한을 30℃로 둔다. DB CHECK `ctrl_thermo_command_temp_range`도 0~30 (신규 INSERT).
- deviation: 5~100 (0.5~10.0℃), UI 스텝 **0.1℃** (최소 0.5℃)
- vent: 0~100, UI 스텝 **1%**, `min_vent_pct` ≤ `max_vent_pct`
...
- **폐기 (신규 uplink):** header `t`, `lut_ver`, v0x0A `replay` 플래그
- **raw 적재 (RS):** uplink **passthrough** — `payload_bytea`만 저장; wire 메타는 RS가 **기록하지 않음**
- **정제 (View):** §4.3.2 — `payload_bytea`에서 controller·`channels[]`·`mesureDt` 추출
- **레거시 burst:** `session_id`·multi-chunk — View/SQL 호환 디코드만 유지
...
| wire 슬롯 | decoded `channel` | 공장 출하 예시 NVM | decoded `eqpmnCode` |
|-----------|-------------------|-------------------|---------------------|
| 1번째 | **A** | `0x03` → EC03 | LIVE 원문 (슬롯과 독립) |
| 2번째 | **B** | `0x02` → EC02 | LIVE 원문 (슬롯과 독립) |
| 3번째 | **C** | `0x01` → EC01 | LIVE 원문 (슬롯과 독립) |

- `channel`(A/B/C)과 `eqpmnCode`는 **독립**. 다른 슬롯이 같은 장비코드를 쓸 수 있다 (예: A·B·C 모두 EC02).
- 공장 출하 표는 **예시**일 뿐, 대시보드가 슬롯으로 장비코드를 채우지 않는다. LIVE `channels[].eqpmnCode`가 있으면 그대로 싣고, 없어도 `SET_CHANNEL_THERMO`는 나간다. 값이 있는데 `EC01`–`EC99`가 아니면 `invalid_eqpmn_code`.
...
**48 ctrl** 1주기: MQTT **48건** (`n=1` × 48) → raw **48 row**.

- 통신모듈: RS-485 read **즉시** uplink (§3)
- RS: row마다 INSERT (**merge 없음**)
- View `v_iot_decoded_latest`: **`controllerKey`별 최신** 1건 — Edge decode → LIVE UI

#### LIVE multi-chunk (레거시·호환)

**48 ctrl** burst: chunk0 **26** + chunk1 **22** (`last_chunk`). View/SQL에서 `session_id`·`chunk_seq` 병합 규칙 유지.
...
```mermaid
flowchart LR
  UI[대시보드] --> CMDDB[(ctrl_thermo_command)]
  CMDDB --> C[C.py]
  C --> MQTT[sungil/…/cmd]
  MQTT --> CM[통신모듈]
  CM --> RAW[sungil/…/raw n=1]
  RAW --> RS[RS passthrough]
  RS --> RAWDB[(iot_room_state_raw)]
  RAWDB --> ACK[command_ack]
  ACK --> CMDDB
  RAWDB --> VIEW[DB View]
```
...
| 단계 | 담당 | 비고 |
|------|------|------|
| insert | dashboard | `status=pending` |
| publish | **C.py** + `wire_command.py` | dedupe: farm+stall+eqpmn+**channel+eqpmn_code** |
| subscribe | 통신모듈 / `sim_fleet` | `…/cmd` QoS 1 |
| uplink | 통신모듈 | cmd 적용 후 **`/raw` 1건** (적용완료 사인) |
| raw 저장 | **RS** | passthrough |
| applied | **command_ack** (RS raw 저장 후) | thermo 4값 일치 · §4.3.4 ⑤ |

명령 상태: `pending` → `sent` (C.py) → `applied` (uplink thermo) · `failed` / `cancelled`
...
- 기본 `cmd_trigger_uplink=true`: cmd 적용 후 **즉시 `/raw` 1건** (RS·`command_ack` E2E)
...
| 구현 | View/연계 레이어 (`farm_item_stall_lut.py` 규칙 참조) |
...
- 메타: `stallLutApplied`, `stallLutRemapped`, `stallLutSource` (View/연계).
...
> **적용 위치:** 실시간 **D.py 프로세스가 아니라 DB View/SQL** (§4.3.2). 규칙 자체는 `wire_decode.py`·§6.13과 동일.
...
7. **LIVE row 스트림:** View `v_iot_decoded_latest` — Edge decode **`controllerKey`별 최신**. cursor lag는 Health `db.decode.lag`
8. **(선택)** `APPLY_STALL_LUT` — View 또는 연계 레이어 (§7.4)
...
| **admin UI·Health** | [Diagrams/deploy/health-monitoring-spec.md](../Diagrams/deploy/health-monitoring-spec.md) |
...
| 1.8 | 2026-06-08 | **raw passthrough:** uplink **그대로** `iot_room_state_raw` 저장 |
| 1.9 | 2026-06-08 | **RS·DB·C 파이프라인:** row 스트림 uplink · **D 폐기** · View 정제 · §4.3 예시 데이터 · C downlink + `command_ack` applied |
