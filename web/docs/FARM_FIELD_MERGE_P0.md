...
차트(TOUR_VERSION 42+): 축사유형 평균 → 유형 칸 → 축사번호 평균 → 축사 펼침(컨트롤러 겹침) → 표시 레이어(카드 상단, 컨트롤러 번호 켜기/끄기) → 온·습 상·하한 → 목록은 30일, 펼친 카드는 휠. 축사 비교는 보류. 차트에서 컨트롤러 명령은 보내지 않음. 델린 뱃지는 차트에서 숨김.
델린·모바일 시트 루트는 후속.
