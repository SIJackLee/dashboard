...
헤더 물음표 → **현재 탭** 스코프.
현장 PC(TOUR_VERSION 24+): 상단 도구 → 보기 탭 → 좌측 현황 → 축사 카드 → 목록 도구 → 컨트롤러 → 그래프 → 설정 → 다시 보기.
현장 모바일(TOUR_VERSION 24+): 상단 도구 → 하단 보기 탭 → 맵 카드 그리드 → 축사 카드 → 일괄적용(권한 시) → 컨트롤러 시트(게이지) → 시트 하단 추이 → 설정 탭 → 다시 보기. 시트 개폐는 `field-mobile-sheet-*` 그리드 액션.
