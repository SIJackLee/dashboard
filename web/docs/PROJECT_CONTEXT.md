...
- **그래프 기간:** `?trendPeriod=24h|7d|30d` (기본 **24h**) — 그리드·목록·차트 공유. 기본 진입은 24시간 데이터만 요청하며, 7일·30일을 선택할 때 30일 데이터를 지연 로드한다. 펼친 카드는 휠·핀치로 폭만 변경. 특정 구간은 플롯 드래그 줌. 룩백 창이 **약 48시간 이하**일 때만 그 구간 15분을 추가 요청. 그리드 히트맵=컨트롤러 추이에서 파생한 축사 평균, 목록/상세=TrendChart 컨트롤러 단위. 버킷 기준 **`mesure_at`**. **로드:** 필드 진입 시 백그라운드. 희소 compact. **24h 15분 → 30d 1시간(하루 RPC로 분할, 최신부터). 7일은 30일 슬라이스.** 히트맵은 GRAPH_BARS(24/28/30) 다운샘플, 차트 탭은 플롯 px viewBox + LTTB(약 2.5px/점). 필드 라인은 점 마커 없음(핀·카드는 유지), 구간 줌·브러시 없음. **PDF·목록 all-periods도 허브와 같은 축**(24h×96 15분 / 7d×168 1h / 30d×720 1h). 인쇄만 LTTB(최대 96점). 15분 2880 경로는 폐기. 줌 창만 `TREND_15M_PERIODS`.
