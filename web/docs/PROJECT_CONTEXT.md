...
- **그래프 기간:** `?trendPeriod=24h|7d|30d` (기본 **7d**) — 그리드·목록·차트·DELIN 탭 공유. 그리드 히트맵=축사 평균(`farm_trend_history`), 목록/상세=TrendChart 컨트롤러 단위(`farm_trend_history_by_controller`). 버킷 기준 **`mesure_at`** (없으면 `received_at`).
