...
| 모델 | `view=model`. 로컬·Preview. 위성 구획 → 1m 필드 → 새 건물 배치 → **배치 완료** 후 동은 세로로 세워 가로로 모이고, **방 선택** 윈도우를 떼면 **연결**/**해제**. 옛 `view=plan`은 같은 탭. [`BARN_PLAN.md`](../BARN_PLAN.md) |
...
- `view=aria` · `view=jarvis` → 필드(`view` 없음)로 정규화. 델린은 상단 탭이 아니라 필드·차트·모델 **우측 하단 뱃지**입니다.
