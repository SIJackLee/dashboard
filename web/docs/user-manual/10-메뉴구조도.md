...
| 차트 | `view=chart` (+ `chartSp`/`chartStall`/`chartCtrl`/`chartW1`/`chartW2`) |
| 모델 | `view=model`. 위성 구획 → 1m 필드 → 새 건물 배치 → **배치 완료** 후 동은 세로로 세워 가로로 모이고, **축사 연결** / **컨트롤러 연결**, 우측 **축사 해제** / **컨트롤러 해제**(일괄). 생성에서 칸 색은 **알람 | 권장**(기본 알람). 옛 `view=plan`은 같은 탭. Production은 관리자만. [`BARN_PLAN.md`](../BARN_PLAN.md) |
...
flowchart TD
  ...
  %% 상단 세 꼭짓점: 보기 · 도구 · 상세
  farm --> adminHub["관리자 허브(농장 미선택) — 지도·현황 관제"]
  farm --> view["보기"]
  farm --> tools["도구"]
  farm --> detail["상세"]
...
- `view=aria` · `view=jarvis` → 필드(`view` 없음)로 정규화. 델린은 상단 탭이 아니라 필드·차트 **우측 하단 뱃지**입니다. 모델 탭에서는 숨깁니다.
