...
- **하단 도크**: **말하기** · **추천 칩**이 기본. **글로 묻기**·**옵션**(읽어주기·장치 테스트)은 접혀 있습니다.
- **답변 카드**: 한 문장 답 + 근거 칩 + **차트에서 보기**(해당 집계 범위로 이동).
- **차트 companion (PC)**: 차트 탭에서 **DELIN** 버튼을 누르면 우측 패널이 열려, 탭을 바꾸지 않고 질문할 수 있습니다.
- **차트 companion (모바일)**: 차트 탭 **DELIN** 버튼 → **하단 시트**에서 말하기·칩·답변. 설정·정밀 조작은 목록 시트를 사용합니다.
