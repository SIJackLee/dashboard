-- FARM trend P2b: aggregate measurements first and decode channel thermo once
-- per controller bucket instead of once per decoded sample.
...
as $function$
  ...
  latest_thermo as (
    select
      l.*,
      public.extract_channel_thermo(
        l.decoded_json -> 'channels',
        'B'
      ) as b_th,
      public.extract_channel_thermo(
        l.decoded_json -> 'channels',
  ...
  select
    ...
    s.avg_fan_c,
    s.sample_count,
    l.setpoint_temp as a_setpoint_temp,
    l.temp_deviation as a_temp_deviation,
    l.min_vent_pct as a_min_vent_pct,
    l.max_vent_pct as a_max_vent_pct,
    nullif(l.b_th ->> 'setpointTemp', '')::numeric as b_setpoint_temp,
    nullif(l.b_th ->> 'tempDeviation', '')::numeric as b_temp_deviation,
    nullif(l.b_th ->> 'minVentPct', '')::numeric as b_min_vent_pct,
    nullif(l.b_th ->> 'maxVentPct', '')::numeric as b_max_vent_pct,
    nullif(l.c_th ->> 'setpointTemp', '')::numeric as c_setpoint_temp,
    nullif(l.c_th ->> 'tempDeviation', '')::numeric as c_temp_deviation,
    nullif(l.c_th ->> 'minVentPct', '')::numeric as c_min_vent_pct,
    nullif(l.c_th ->> 'maxVentPct', '')::numeric as c_max_vent_pct
  from stats s
...
comment on function public.farm_trend_history_by_controller(
  ...
  'Controller trend averages plus A/B/C thermo decoded once from the latest sample in each bucket.';
