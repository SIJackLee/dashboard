-- Trend RPC: bucket/filter by received_at (cloud arrival), not mesure_at.
-- Controllers that stamp local wall-clock as UTC unix sit ~9h ahead of now(),
-- so a mesure_at window ending at now() dropped every live sample from the chart.
...
comment on function public.farm_trend_history(text, text, timestamptz, timestamptz, interval) is
  'Dashboard trend: bucketed stall avg by received_at. Includes live/history/replay. SECURITY INVOKER → RLS.';
...
comment on function public.farm_trend_history_by_controller(text, text, timestamptz, timestamptz, interval) is
  'Dashboard list/chart: controller-level avg by received_at. Includes live/history/replay. SECURITY INVOKER → RLS.';
