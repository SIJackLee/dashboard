import { resolveUltraFcstBase, resolveUltraNcstBase } from "./kma-base-time.ts";
...
async function fetchKmaJson(
  ...
  try {
    return { ok: true, json: (await res.json()) as KmaApiResponse };
  } catch {
...
async function fetchUltraNcst(
  ...
  if (header.resultCode !== "00") {
    return { ok: false as const, error: "kma_result", header };
  }
  return {
    ok: true as const,
...
export async function fetchKmaReading(
  ...
  if (!ncst.ok && ncst.header?.resultCode === "03") {
    ...
    if (ncst.ok) Object.assign(ncstBase, fallbackBase);
  }
  ...
  const ncstParsed = ncst.ok
    ? parseNcstItems(ncst.items)
    : { tempC: null, humidityPct: null, windMs: null, precipMm: null };
