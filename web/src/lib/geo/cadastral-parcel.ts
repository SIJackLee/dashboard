...
import { cadastralLotLabel, isCadastralSiteNeighbor, vworldLonLatBox } from "@/lib/geo/cadastral-pnu";
...
export async function lookupCadastralBoundary(
  ...
  try {
    const features = await vworldGetFeatures({
      key,
      domain,
      data: CADASTRAL_DATA,
      size: "100",
