    ...
    <div className="flex min-w-0 flex-1 flex-wrap items-center gap-3">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <h2
            className={cn(
              compact ? "text-lg font-semibold" : dashboardTypography.sectionTitle
            )}
          >
        ...
        {compact ? null : (
          <p className={cn(dashboardTypography.meta, "mt-1")}>{HEALTH_UI.systemDesc}</p>
        )}
        {compact || !impactScope ? null : (
          <p
            className={cn(
              compact ? "text-sm text-muted-foreground" : dashboardTypography.body,
              compact ? "mt-0.5" : "mt-1"
            )}
          >
