...
import { FIRMWARE_CTRL_COUNT, FIRMWARE_CTRL_IDX_MAX } from "@/lib/data/iot-firmware";
...
function catalogKey(e: StallCatalogEntry) {
  ...
  return stallCatalogKey(e.farmKey, e.moduleUid, e.stallNo);

}



function barnCatalogKey(b: BarnMeta) {

  return stallCatalogKey(b.farmKey, b.moduleUid, b.stallNo);
...
function newBarn(
  ...
  const used = new Set(existing.map(barnCatalogKey));
  ...
  return {
    ...
    farmKey: next?.farmKey ?? DEFAULT_FARM,
...
export function BarnMetaForm({ initialBarns, stallCatalog, notice }: Props) {
  ...
  const updateFarmKey = (id: string, patch: Partial<FarmKey>) => {

    setBarns((prev) =>

      prev.map((b) =>

        b.id === id ? { ...b, farmKey: { ...b.farmKey, ...patch } } : b
  ...
  const catalogOptionsForBarn = (barn: BarnMeta) => {
    ...
    const usedByOthers = new Set(
      ...
      barns.filter((b) => b.id !== barn.id).map(barnCatalogKey)
    ...
    return stallCatalog.filter(
      ...
      (c) =>
        ...
        !usedByOthers.has(catalogKey(c)) || catalogKey(c) === barnCatalogKey(barn)
  ...
  return (
    ...
    <form action={saveBarnMetasAction} className="space-y-4">
      ...
      <SectionCard
        ...
        description={`농장(farm) 내 축사(stallNo)를 지정합니다. 축사 하나에 컨트롤러 여러 대가 연결될 수 있으며, 통신박스당 컨트롤러는 최대 ${FIRMWARE_CTRL_COUNT}대(idx 0~${FIRMWARE_CTRL_IDX_MAX})입니다.`}
          ...
          <div className="space-y-6">
            ...
            {barns.map((barn) => {
              ...
              const selectValue =
                ...
                options.some((c) => catalogKey(c) === barnCatalogKey(barn))
                ...
                <div key={barn.id} className="rounded-lg border p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                          ...
                          <SelectContent>
                            ...
                            {options.map((c) => (
                              ...
                              <SelectItem key={catalogKey(c)} value={catalogKey(c)}>
                                ...
                                {c.stallTyCode
                                  ? ` (${formatStallTypeLabel(c.stallTyCode)})`
                                  : ""}

                                {" · "}

                                {farmShortLabel(c.farmKey)} / 통신박스{" "}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Label>축산업 등록번호</Label>

                      <Input

                        type="text"

                        value={barn.farmKey.lsindRegistNo}

                        onChange={(e) =>

                          updateFarmKey(barn.id, {

                            lsindRegistNo: e.target.value.trim(),
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Label>
                        축종 코드 (itemCode)
                        {barn.farmKey.itemCode
                          ? ` · ${formatItemCodeLabel(barn.farmKey.itemCode)}`
                          : ""}
                      </Label>
                      ...
                      <Input
                        ...
                        value={barn.farmKey.itemCode}

                        onChange={(e) =>

                          updateFarmKey(barn.id, { itemCode: e.target.value.trim() })
