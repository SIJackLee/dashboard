...
| View (목표) | 역할 |
|-------------|------|
| `v_iot_raw_packets` | raw + header 최소 필드 (`wire_ver`, `n`, `byte_len`) |
| `v_iot_controller_row` | `payload_bytea` → **controller 1행** (위치·`mesureDt`·`channels[]`) |
| `v_iot_live_latest` | `controllerKey`별 **최신 1건** (`DISTINCT ON` + `received_at DESC`) |

디코드 규칙은 §8.1과 동일. **실행 주체는 D.py가 아니라 View/SQL 함수.**

대시보드·알람·연계는 **`iot_room_state_decoded` 쓰기 없이** View SELECT.
...
**⑥ LIVE 화면 (View `v_iot_live_latest`)**

모듈 FARM01/P00 기준 `controllerKey`별 최신 row 48건을 merge — 패널에는 ③과 동일 구조로 표시.
...
- View `v_iot_live_latest`: **`controllerKey`별 최신** 1건으로 LIVE UI 구성
...
7. **LIVE row 스트림:** View `v_iot_live_latest` — **`controllerKey`별 최신** (`received_at` 또는 `mesureDt`). 레거시 burst는 `session_id` 병합 (호환)
