...
> **현행 (펌웨어·RSD):** wire `ver=0x0A` / `lut_ver=11` — [명세](../RSD/docs/wire-v00a-spec.md)  
> **이전:** `ver=0x08` / `lut_ver=9` — [v008](../docs/changes/20260611-wire-v008-thermo-spec.md) · `ver=0x07` / `lut_ver=8` — [v007](../docs/changes/20260610-wire-v007-spec.md) · `ver=0x06` — [v006](../docs/changes/20260610-wire-v006-spec.md)  
> **레거시:** `P2T2F30` / `ver=0x03`·`0x04` / `lut_ver=7` · `ver=0x02` / `lut_ver=6` (fan_bits, 614 B)  
> **decoded_json:** `ES01`·`ES02`·`EC01`~`EC03` **배열형** (§8.2) · **`controllerKey`** (schema **1.8**, §8.2.3) · **`thermo`** (§8.2.1)
...
| **컨트롤러(ctrl)** | **업무 키:** `controllerKey` = `{stallTyCode}:{stallNo}:{eqpmnNo}` (예: `SP07:03:02`). wire row byte0 = **`eqpmn_no` 1~10** (축사 내 장비번호). RS-485 `ctrl_idx`는 **통신모듈 내부 전용** — uplink/downlink JSON·DB에 실리지 않음 |
...
> **현행 on-wire:** `ver=0x0A` (§6.12). §6.9~6.11·§6.1~6.8은 **이전·레거시** 참조.
...
2. body CRC 검증
3. Header 파싱 (`ver=0x0A`: v0x07/v0x08과 동일 13 B header)
4. `lut_ver`로 레거시 LUT 로드 (v0x03 ctrl 매핑 등, 선택)
5. 각 row → **ctrl 1건** `decoded_json.controllers[]`:
   - **`ver=0x0A`:** byte0 `eqpmn_no` → `eqpmnNo`, `stall_ty`/`stall_no` → `stallTyCode`/`stallNo`, **`controllerKey`** (§8.2.3). **`idx` 생략**
   - **`ver=0x09` 이하:** `ctrl_idx` → `idx`, `eqpmnNo` = idx+1 (레거시)
   - `es01[2]` → `"ES01": […]` (÷10, `0xFFFF` → `null`)
   - `es02[2]` → `"ES02": […]`
   - `ec03_out` / `ec02_out` / `ec01_out` → `"EC03"` / `"EC02"` / `"EC01"` 배열
   - **`ver=0x08+`:** row thermo tail → `"thermo"` (§8.2.1). NA면 **생략**
6. `t` / row `row_t` → `mesureDt` (KST 등 timezone 정책은 서버 설정)
7. **LIVE multi-chunk:** `last_chunk`까지 수신 후 controllers 병합 (§6.12). v0x0A는 **`controllerKey`** 병합 키
8. **(선택)** `APPLY_STALL_LUT=true` → `controllers[].stallTyCode` 축종 LUT remap (§7.4)
...
### 8.2.1 `thermo` 객체 (schema **1.6+**, `ver=0x08+`)

controller 레코드 **선택 필드**. downlink 설정·RS485 readback 실값 uplink. v0x0A(schema **1.8**)에서도 동일.
...
- **대시보드:** LIVE `thermo` 우선; 없으면 `ctrl_thermo_command` fallback
- **적용 확인:** LIVE uplink thermo가 sent 명령과 일치하면 `status=applied` (RSD `command_ack.py`, **v0x0A는 stall+eqpmn 매칭**)

### 8.2.3 `controllerKey` · v0x0A 식별 (schema **1.8**)
...
- **대시보드·DB·downlink**는 `controllerKey` (또는 3필드) 사용
- **레거시 v0x09 이하:** `idx` + `eqpmnNo`=idx+1, `controllerKey` 없음 → UI는 `legacy:idx:N` fallback
- **chunk merge (RSD·dashboard):** v0x0A = `controllerKey`, 레거시 = `idx`
...
### 8.3 예시 — v0x0A controller `SP05:03:02`
...
| ctrl 설치 | **48대** | LIVE multi-chunk 합산, `FIRMWARE_LIVE_ROW_COUNT_MAX=48` (RSD) |
| wire `ver` | **`0x0A`** | row 51 B, byte0=`eqpmn_no`, schema **1.8**, `controllerKey` |
| `lut_ver` | **`11`** | v0x0A stall+eqpmnNo 식별 |
| decoded schema | **1.8** | `controllerKey`; **`idx` 없음** |
...
| 문서 | 경로 |
|------|------|
| **wire v0x0A (현행)** | [RSD/docs/wire-v00a-spec.md](../RSD/docs/wire-v00a-spec.md) |
